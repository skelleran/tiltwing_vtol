...
//THIS CODE IS A MODIFIED VERSION OF NICHOLAS REHM's DREHMFLIGHT VTOL CODE: 
//https://github.com/nickrehm/dRehmFlight

//My version operates an independent tiltwing with tail elevator, and can switch between hover, transition, and forward flight modes.

//========================================================================================================================//

//CREDITS (from drehmflight source code)
/*
...
float Ki_roll_angle = 0.7;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.07;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.4;   //Pitch P-gain - angle mode (Default 0.2)
...
float Kp_roll_rate = 0.3;    //Roll P-gain - rate mode
float Ki_roll_rate = 0.2;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.0002;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
float Kp_pitch_rate = 0.3;   //Pitch P-gain - rate mode (Default 0.15)
...
void controlMixer() {
  ...
  float s1_trim_hover = 0.75;
  float s2_trim_hover = 0.27;

  float s1_trim_forward = 0.15;
  float s2_trim_forward = 0.9;

  float s1_trim_trans = (s1_trim_hover + s1_trim_forward)/2;
  float s2_trim_trans = (s2_trim_hover + s2_trim_forward)/2;
  ...
  if (channel_5_pwm > 1500){
    ...
    s1_command_scaled = s1_trim_hover - 2*pitch_PID + yaw_PID;
    s2_command_scaled = s2_trim_hover + 2*pitch_PID + yaw_PID;
    s3_command_scaled = s3_trim;
  }
    ...
    //0.5 is centered servo, 0 is zero throttle if connecting to ESC for conventional PWM, 1 is max throttle
    s1_command_scaled = s1_trim_forward + roll_PID;
    s2_command_scaled = s2_trim_forward + roll_PID;
    s3_command_scaled = s3_trim + 2*pitch_PID;
  }

  else {
    //TRANSITION MODE
    m1_command_scaled = thro_des + 0.5*yaw_PID - 0.5*roll_PID;
    m2_command_scaled = thro_des - 0.5*yaw_PID + 0.5*roll_PID;
    
  
    //0.5 is centered servo, 0 is zero throttle if connecting to ESC for conventional PWM, 1 is max throttle
    s1_command_scaled = s1_trim_trans - pitch_PID + 0.5*yaw_PID + 0.5*roll_PID;
    s2_command_scaled = s2_trim_trans + pitch_PID + 0.5*yaw_PID + 0.5*roll_PID;
    s3_command_scaled = s3_trim + 1.5*pitch_PID;
  }
